 ...
 *
 * Usage:
 *   node config/scripts/perf_baseline_gate.js                  # compare (default)
 *   node config/scripts/perf_baseline_gate.js --update-baseline   # rewrite baseline
 *
...
function loadBaseline() {
  ...
  try {
    ...
    console.error('[perf] config/scripts/perf_baseline.json is malformed (missing entries map). Run with --update-baseline to rewrite.')
    return null
  } catch (err) {
    console.error(`[perf] Failed to parse config/scripts/perf_baseline.json: ${err.message}`)
    return null
  }
}
...
function modeUpdateBaseline() {
  ...
  if (!measurement) {
    console.error(`[perf] Build directory ${BUILD_DIR_REL} is empty or missing. 请先跑 yarn prod 再更新基线。`)
    process.exit(1)
  }
  ...
  console.log('[perf] 请记得 git add config/scripts/perf_baseline.json 并 commit。')
}
...
function modeCompare() {
  ...
  if (!baseline) {
    ...
    writeBaseline(measurement)
    const total = measurement.entries[TOTAL_KEY].bytes
    console.warn(`[perf] WARN: 未找到 config/scripts/perf_baseline.json，已基于当前构建生成首次基线（总大小 ${formatBytes(total)}）。`)
    console.warn('[perf] 请 git add config/scripts/perf_baseline.json 并 commit，否则下次构建仍会重新生成。')
    return
  }
  ...
  if (failures.length) {
    ...
    console.error('')
    console.error('[perf] 如果这是预期变化，跑 `node config/scripts/perf_baseline_gate.js --update-baseline` 更新基线，然后 commit config/scripts/perf_baseline.json。')
    process.exit(1)
  }
