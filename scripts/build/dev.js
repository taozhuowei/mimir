 ...
 *   2. quality gate (full) — same as pre-push, catches issues before
 *      a long-running watch eats your terminal
 *   3. concurrently start watchers per --target:
...
module.exports = async function devPipeline({ targets, skipQuality }) {
  ...
  // Step 2: quality (skippable for symmetry with prod).
  if (!skipQuality) {
  ...
  // Step 3: long-running watchers under concurrently.
