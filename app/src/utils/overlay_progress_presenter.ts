/**
 * Name: overlay_progress_presenter
...
import { getPhaseStep, PHASE_STEPS } from './overlay_phase_registry'
...
export function presentProgressHeader(
  ...
  const activeIndex = PHASE_STEPS.findIndex((s) => s.phase === currentPhase)

  const items = PHASE_STEPS.map((step, index) => {
