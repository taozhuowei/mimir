/**
 * Name: flows/base/composables/use_header_presentation
 * Purpose: derive the top header region's variant + ARIA + slide-in style
 *          from the application flow and the animation controller:
 ...
 * Data flow: flow ref + animationController ──▶ reactive role / aria /
...
export interface HeaderPresentation {
  ...
  /** `banner` in idle, `progressbar` in every divination flow. */
  headerRole: ComputedRef<'banner' | 'progressbar'>
  /**
   * Active progress step label (e.g. "审视" / "命定") surfaced as
   * aria-valuetext in non-idle flows; `undefined` in idle (no progress bar).
   */
  headerAriaValuetext: ComputedRef<string | undefined>
  /**
   * The GSAP slide-in tween style, bound only in non-idle flows. In idle
