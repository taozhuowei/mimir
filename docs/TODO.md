...
四类搁置债转为 F1–F6 执行计划。F1（pivot 抽屉措辞残留清理）提交 `a2e82a7`；F2（中文「解读」逐条甄别）零文件需改 `8f985fd`；F3（DevToolsPanel 拖拽接线抽 composable）提交 `51bd81e`；F4（TitleContent 入场动画抽 composable）已全套验收（full 门禁 + e2e 15/15）待提交。余 F5（isWide 布局逻辑债）/F6（cardWidthFull 合一）——开工即停下报告，需产品决策/评估。
